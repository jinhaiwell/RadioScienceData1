...
function [rcs,rcs_n,Ercs,Ercs_n,phi,phi_n] = rcssphere(r,c,fc,az,el,snr)
...
Ercs=nrcs_vec;%%complex electric filed
Ercs_n=awgn(nrcs_vec,snr);%%complex electric filed with noise
phi_n=angle(Ercs_n);
phi=angle(nrcs_vec);
nrcs_vec_n = abs(Ercs_n).^2./(kr.^2);
nrcs_vec = abs(nrcs_vec).^2./(kr.^2);
rcs_n = ones(size(fcg)).*pi*r.^2.*permute(nrcs_vec_n,[1 3 2]);
